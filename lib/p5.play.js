...
function Animation(pInst) {
  ...
  */
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    if(ext1 !== '.png' && ext1 !== '.svg')
    {
      pInst.print('Animation error: you need to use .png or svg files (filename '+from+')');
      from = -1;
    }

    if(ext1 !== ext2)
    {
      pInst.print('Animation error: your starting and ending files must be of the same type (filename '+to+')');
      to = -1;
    }
      ...
      else
      {
        ...
        if(digits1 === digits2)
        {
          ...
          for (i = number1; i <= number2; i++) {
            ...
            fileName = prefix1 + pInst.nf(i, digits1) + ext1;
